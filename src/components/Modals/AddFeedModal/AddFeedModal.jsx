...
import * as S from './AddFeedModalStyle';
...
function SelectFeedImg({setPage, setFiles}) {
    ...
    return (
        <div css={S.SelectFeedImgContainer}>
            ...
            <button css={S.SBtn} onClick={handleSelectImg}>컴퓨터에서 선택 </button>
...
function ReviewFeedImg({ files }) {
    ...
    return (
        <div css={S.ReviewContainer}>
            ...
            <Carousel
                showArrows={true} // 화살표
                autoPlay={false} // 자동 이동 
                infiniteLoop={false} //맨끝에서 다시 처음으로 이동되게    
                showThumbs={false}>  
                {/* 썸네일 */}
...
function AddFeedModal() {
    ...
    const [bodyComponent, setBodyComponent] = useState(<SelectFeedImg setPage={setPage} setFiles={setFiles} />);

    const [leftBtn, setLeftBtn] = useState(<div></div>);
    const [rightBtn, setRightBtn] = useState(<div></div>);
    ...
    return (
        <div>
            <ModalLayout>
                <ModalHeader title={"새 게시물 만들기"} leftBtn={leftBtn} rightBtn={rightBtn} />
                <ModalBody>
                    {bodyComponent}
                </ModalBody>
            </ModalLayout>
